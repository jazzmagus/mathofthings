---
title: "La musica contro l'ansia: la _Math-Music Bubble_"
subtitle: "Come la musica può diventare un ausilio compensativo per gli studenti BES, e uno studio di caso nelle verifiche di matematica"
summary: "L'ansia scolastica affligge sempre più studenti, in particolare quelli con Bisogni Educativi Speciali. La musica può aiutarli a gestirla: dai dati sullo stato dell'arte fino a uno studio di caso reale, la Math-Music Bubble come strategia inclusiva nelle verifiche di matematica."
authors: [diego fantinelli]
tags: [inclusione, BES, musica, didattica, matematica]
categories: [lesson]
...
- Anzitutto, la musica distrae dallo stress. Quando ascoltiamo della _buona musica_, l'attenzione si sposta su ciò che sentiamo invece che sui pensieri che ci assillano: un modo semplice per spezzare quel circolo di pensieri negativi che spesso alimenta l'ansia.
...
Per Pitagora — a cui dobbiamo proprio l'intuizione di _rappresentare matematicamente i suoni_ — la musica aveva tre orientamenti:

1. di **adattamento**: la musica deve conformarsi alla personalità dell'individuo, che a sua volta deve sforzarsi di accogliere anche musiche lontane dalla propria indole;
2. di **cambiamento**: la musica può modificare lo stato d'animo, aiutando a comprendere e potenziare le proprie capacità e ad accettare i propri limiti;
3. di **purificazione**: la musica libera l'animo dalle tensioni, come una medicina fa con il corpo.

L'idea di intrecciare musica e matematica nasce quindi dalla mia passione — e dai miei studi — per entrambe le discipline. L'occasione di sperimentarla direttamente, usando la musica come ausilio compensativo, si è presentata di recente; e, sperando di poterla riproporre in futuro, ho voluto darle un nome: _Math-Music Bubble_.
...
- costruire una sorta di bolla, una _comfort zone_ in cui rifugiarsi, al riparo dall'inquietudine che quel silenzio potenzialmente ansiogeno — così tipico di una verifica di matematica — porta con sé.
...
- una o più figure di coordinamento — di solito il coordinatore e, all'occorrenza, un _docente tutor_ indicato dallo studente stesso;
...
(function () {
  ...
  function build() {
    ...
    // 1 — Prevalenza ansia/depressione adolescenti
    instances.push(new Chart(document.getElementById('mmChart1'), {
      type: 'bar',
      data: {
        labels: ['Sintomi d’ansia', 'Sintomi depressivi'],
        datasets: [{
          data: [20.5, 25.2],
          backgroundColor: [t.coral, t.slate],
          borderRadius: 6
        }]
      },
      options: Object.assign({}, baseOpts, {
        scales: {
          y: Object.assign(scale({ beginAtZero: true, max: 30 }), {
            ticks: { color: t.text, callback: function (v) { return v + '%'; } },
            title: { display: true, text: 'Adolescenti (%)', color: t.text }
          }),
    ...
    // 2 — Effetto della musica sullo stress (d di Cohen)
    instances.push(new Chart(document.getElementById('mmChart2'), {
      type: 'bar',
      data: {
        labels: ['Effetto fisiologico', 'Effetto psicologico', 'Musicoterapia (stress)'],
        datasets: [{
          data: [0.38, 0.55, 0.72],
          backgroundColor: [t.slate, t.ochre, t.coral],
          borderRadius: 6
        }]
      },
      options: Object.assign({}, baseOpts, {
        indexAxis: 'y',
        scales: {
          x: Object.assign(scale({ beginAtZero: true, max: 1 }), {
            title: { display: true, text: 'd di Cohen', color: t.text }
          }),
    ...
    // 3 — Trend DSA in Italia
    instances.push(new Chart(document.getElementById('mmChart3'), {
      type: 'line',
      data: {
        labels: ['2010/11', '2016/17', '2017/18', '2019/20'],
        datasets: [{
          data: [0.7, 2.9, 3.2, 5.3],
          borderColor: t.coral,
          backgroundColor: t.fill,
          borderWidth: 3, fill: true, tension: 0.3,
          pointRadius: 5, pointBackgroundColor: t.coral,
          pointBorderColor: t.cardBg, pointBorderWidth: 2
        }]
      },
      options: Object.assign({}, baseOpts, {
        scales: {
          y: Object.assign(scale({ beginAtZero: true, max: 6 }), {
            ticks: { color: t.text, callback: function (v) { return v + '%'; } }
          }),
          x: { ticks: { color: t.text }, grid: { display: false } }
        }
  ...
  function start() {
    ...
    if (window.__mmChartsDrawn) return;
    window.__mmChartsDrawn = true;
    build();
    var pending;
